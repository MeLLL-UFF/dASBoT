 ...
 *
 */
public class CrossValidationPreDefinedFolds extends CrossValidation {
	...
	public CrossValidationPreDefinedFolds(){
		
	}
	
	public CrossValidationPreDefinedFolds(AbstractCELA la, PosNegLP lp, AbstractReasonerComponent rs, String prefixFile){
	...
	public void runCV(int folds, String nameOutput){		
	...
	public void runCVWithValSet(int folds, String nameOutput) {		
		...
		for (int fold = 1; fold <= folds; fold++){
			...
			outputWriter(" Best concepts found: " + concept);
	...
	public void runCVInternalVal(int kFolds, int xFolds, String prefixFile, String evalfn, String nameOutput) {		
		...
		for (int kfold = 1; kfold <= kFolds; kfold++){
			...
			for (int xfold = 1; xfold <= xFolds; xfold++){
				...
				else if (evalfn.equals("accuracy")){
